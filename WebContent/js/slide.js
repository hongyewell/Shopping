...
function dslide(id, time) {
	...
	for(var i = 0; i < this.max; i++) {
		this.htm += '<span id="'+id+'_no_'+i+'" style="padding:1px 5px 1px 5px;margin-left:1px;cursor:pointer;font-size:12px;color:#FFFFFF;'+(i == this.c ? 'background:#FF7E00;' : 'background:#009966;')+'">'+(i+1)+'</span>';
	}
	this.htm += '</div>';
	$('#'+id).html(this.htm);
	...
	$('#'+id).mouseout(function() {clearTimeout(a);_this.p=0;});
	$('#'+id).find('span').each(function(i) {
		$(this).mouseover(function() {
			a = setTimeout(function(){_this.slide(i)},500);
		});
	});
	this.slide = function(o) {
		...
		$('#'+id+'_no_'+this.c).css({background:'#009966'});
